...
- This repository is a monorepo managed with `Nx` and `Bun` as the package manager, containing multiple tools and
  packages.

- Each workspace specified with `workspaces` key in `package.json` is a git submodule pointing to its own repository.

- This repository uses Oxlint as the linter. When providing linting commands or configurations, ensure they are
  compatible with Oxlint.

- Commit messages should follow the Conventional Commits specification. Use the Commitizen tool configured in the
  repository to assist with creating properly formatted commit messages.
